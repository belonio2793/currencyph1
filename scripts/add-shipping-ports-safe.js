...
async function addPortsSafely() {
  try {
    console.log('Fetching existing ports...')
    const { data: existingPorts, error: fetchError } = await supabase
      .from('shipping_ports')
      .select('name')

    if (fetchError) {
      console.error('Error fetching existing ports:', fetchError)
      process.exit(1)
    }

    const existingNames = new Set(existingPorts.map(p => p.name))
    const portsToAdd = newPorts.filter(port => !existingNames.has(port.name))

    if (portsToAdd.length === 0) {
      console.log('✅ All ports already exist in the database!')
      console.log(`Total existing ports: ${existingPorts.length}`)
      process.exit(0)
    }

    console.log(`\nFound ${portsToAdd.length} new ports to add out of ${newPorts.length}`)
    console.log('Adding the following ports:')
    ...
    if (error) {
      console.error('\n❌ Error inserting ports:', error)
      process.exit(1)
    }

    console.log(`\n✅ Successfully added ${data.length} ports!`)
    ...
    console.log(`\n📊 Total public shipping ports now: ${allPorts.length}`)
