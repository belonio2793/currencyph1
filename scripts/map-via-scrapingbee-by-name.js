...
async function main(){
  // fetch synthetic listings
  const { data, error } = await supabase.from('nearby_listings').select('id, tripadvisor_id, name, city').like('tripadvisor_id', 'php_%').limit(200)
  ...
  for (const l of listings){
    try{
      ...
      if (links.length===0){
        console.log(`✗ No links for: ${l.name}`)
        await sleep(400)
        continue
      }
      ...
      await sleep(500)
    }catch(e){ console.log(`✗ Error for ${l.name}:`, e.message); await sleep(600) }
  }
