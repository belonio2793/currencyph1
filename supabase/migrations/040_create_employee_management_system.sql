...
CREATE INDEX idx_job_invitations_invited_user ON public.job_invitations(invited_user_id);
CREATE INDEX idx_job_invitations_business ON public.job_invitations(business_id);
CREATE INDEX idx_job_invitations_status ON public.job_invitations(status);
CREATE INDEX idx_job_invitations_sent_at ON public.job_invitations(sent_at DESC);
...
CREATE INDEX idx_employee_assignments_user ON public.employee_assignments(user_id);
CREATE INDEX idx_employee_assignments_business ON public.employee_assignments(business_id);
...
CREATE INDEX idx_employee_assignments_start_date ON public.employee_assignments(start_date DESC);
...
CREATE TABLE IF NOT EXISTS public.employee_attendance (
  ...
  notes TEXT,
  status VARCHAR(50) DEFAULT 'checked_in', -- 'checked_in', 'checked_out', 'absent', 'late', 'early_leave'
  
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

CREATE INDEX idx_employee_attendance_business ON public.employee_attendance(business_id);
CREATE INDEX idx_employee_attendance_employee ON public.employee_attendance(employee_id);
CREATE INDEX idx_employee_attendance_user ON public.employee_attendance(user_id);
CREATE INDEX idx_employee_attendance_date ON public.employee_attendance(attendance_date DESC);
CREATE INDEX idx_employee_attendance_status ON public.employee_attendance(status);
