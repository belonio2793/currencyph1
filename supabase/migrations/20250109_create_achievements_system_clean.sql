-- Drop existing objects if they exist (cascade to handle dependencies)
DROP TRIGGER IF EXISTS trigger_update_player_stats ON public.game_matches CASCADE;
DROP FUNCTION IF EXISTS public.update_player_stats_on_match() CASCADE;
DROP VIEW IF EXISTS public.current_season_leaderboard CASCADE;
DROP VIEW IF EXISTS public.player_achievement_summary CASCADE;
DROP TABLE IF EXISTS public.game_player_achievements CASCADE;
DROP TABLE IF EXISTS public.game_player_stats CASCADE;
DROP TABLE IF EXISTS public.game_seasonal_leaderboard CASCADE;
DROP TABLE IF EXISTS public.game_achievements CASCADE;

-- Achievement definitions (key as PRIMARY KEY)
...
CREATE TABLE public.game_player_achievements (
...
CREATE TABLE public.game_seasonal_leaderboard (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  character_id UUID NOT NULL,
...
-- Create indexes
CREATE INDEX idx_player_achievements_character ON public.game_player_achievements(character_id);
CREATE INDEX idx_player_achievements_unlocked ON public.game_player_achievements(unlocked_at DESC);
CREATE INDEX idx_player_achievements_achievement ON public.game_player_achievements(achievement_key);
CREATE INDEX idx_seasonal_leaderboard_character ON public.game_seasonal_leaderboard(character_id);
CREATE INDEX idx_seasonal_leaderboard_season ON public.game_seasonal_leaderboard(season_number, rank);
CREATE INDEX idx_player_stats_character ON public.game_player_stats(character_id);
