      ...
      {!minimized && (
        <div className={`bg-white shadow-2xl border border-slate-200 flex flex-col ${
          ...
          <div className="flex border-b border-slate-200">
            <button onClick={() => setActiveTab('chats')} className={`flex-1 py-2 text-xs md:text-sm font-medium transition-colors ${activeTab === 'chats' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-slate-600 hover:text-slate-900'}`}>Chats</button>
            <button onClick={() => setActiveTab('friends')} className={`flex-1 py-2 text-xs md:text-sm font-medium transition-colors ${activeTab === 'friends' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-slate-600 hover:text-slate-900'}`}>Friends</button>
            <button onClick={() => setActiveTab('all')} className={`flex-1 py-2 text-xs md:text-sm font-medium transition-colors ${activeTab === 'all' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-slate-600 hover:text-slate-900'}`}>All</button>
          </div>

          <div className={`border-b border-slate-200 ${isMobile ? 'p-2' : 'p-3'}`}>
            <input type="text" placeholder={activeTab === 'chats' ? 'Search conversations...' : activeTab === 'friends' ? 'Search friends...' : 'Search users...'} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className={`w-full border border-slate-300 rounded-lg focus:outline-none focus:border-blue-500 ${isMobile ? 'px-2 py-1.5 text-sm' : 'px-3 py-2 text-sm'}`} />
          </div>
            ...
            {activeTab === 'chats' ? (
              selectedConversation ? (
                <div className="flex flex-col h-full">
                  ...
                  <div ref={messageListRef} className={`flex-1 overflow-y-auto p-3 space-y-3 bg-white rounded-lg transition-colors ${isDraggingOver ? 'border-2 border-dashed border-slate-300' : 'border border-slate-200'}`} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop}>
