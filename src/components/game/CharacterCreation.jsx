...
const MALE_HAIRSTYLES = [
  ...
  { id: 'pompadour', label: 'Pompadour', icon: '🔸' },
  { id: 'slick', label: 'Slick Back', icon: '⬛' },
  { id: 'manbun', label: 'Man Bun', icon: '🟫' },
  { id: 'long_male', label: 'Long', icon: '🟦' },
  { id: 'curly_male', label: 'Curly', icon: '🟢' }
]
...
const FEMALE_HAIRSTYLES = [
  ...
  { id: 'long_female', label: 'Long Flow', icon: '🟦' },
  { id: 'curly_female', label: 'Curly Big', icon: '🟢' }
]
...
export default function CharacterCreation({ onCharacterCreated, userId }) {
  ...
  // RGB inputs state derived from hair_color
  ...
  function handleAppearanceChange(key, value) {
    ...
    if (key === 'hair_style') {
      // set default color per style (creative choices)
      const defaults = {
        buzz: '#2b2b2b',
        pompadour: '#422b18',
        slick: '#1b1b1b',
      ...
      if (defaults[value]) {
        setAppearance(prev => ({ ...prev, hair_color: defaults[value] }))
        setRgb(hexToRgb(defaults[value]))
      }
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 to-slate-800 flex items-center justify-center p-3">
      <div className="w-full max-w-5xl">
        <div className="bg-slate-800 border border-slate-700 rounded-lg p-4 md:p-6 lg:p-8">
          ...
          <p className="text-center text-slate-400 mb-4 md:mb-6">Create Your Adventure — elaborate customization enabled</p>
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            <div className="flex-1">
              <form onSubmit={handlePrepareCreate} className="space-y-3">
                <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    ...
                    <div className="flex gap-2">
                      {['male', 'female'].map(g => (
                        <button
                          type="button"
                          key={g}
                          onClick={() => handleAppearanceChange('gender', g)}
                          className={`flex-1 px-3 py-2 rounded-lg border-2 text-sm ${appearance.gender === g ? 'border-blue-500 bg-blue-500/10 text-blue-300' : 'border-slate-600 text-slate-300'}`}>
                  ...
                  <div>
                    ...
                    <div className="flex gap-2">
                      {[{name:'light',c:'#fdbcb4'},{name:'medium',c:'#d4a574'},{name:'dark',c:'#8b5a3c'},{name:'olive',c:'#9a7c5c'}].map(t => (
                        <button key={t.name} type="button" onClick={() => handleAppearanceChange('skin_tone', t.name)} className={`w-10 h-10 rounded border-2 ${appearance.skin_tone===t.name ? 'ring-2 ring-blue-500 border-blue-500' : 'border-slate-600'}`} style={{backgroundColor:t.c}} title={t.name} />
                      ))}
                    </div>
                  </div>
                </div>

                {/* Hairstyles with icons */}
                <div>
                  <label className="block text-sm font-medium text-slate-300 mb-2">Hairstyles ({appearance.gender})</label>
                  <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2">
                    {hairstyles.map(h => (
                      <button key={h.id} type="button" onClick={() => handleAppearanceChange('hair_style', h.id)} className={`flex flex-col items-center justify-center gap-1 p-2 rounded-lg border-2 ${appearance.hair_style===h.id ? 'border-blue-500 bg-blue-500/10' : 'border-slate-700 bg-slate-900'} text-xs`}>
                        <div className="w-12 h-12 flex items-center justify-center rounded-md overflow-hidden" style={{backgroundColor:'#0b1220'}}>
                          <HairIcon styleId={h.id} color={appearance.hair_color} gender={appearance.gender} />
                        </div>
                        <div className="truncate w-full text-xs text-slate-300">{h.label}</div>
                      </button>
                    ))}
                  </div>
                </div>

                {/* Color palettes and RGB */}
                <div>
                  <label className="block text-sm font-medium text-slate-300 mb-2">Hair Color — pick or fine tune (RGB)</label>
                  <div className="flex gap-2 items-center mb-2">
                    {[ '#1b1b1b','#4b2e2e','#6b3a2a','#c09c6b','#f1c27d','#f9e0c7','#2e4b6b','#5a2f5a','#ffffff','#ff0000'].map(c => (
            ...
            <div className="w-full lg:w-96 flex-shrink-0">
              <div className="bg-slate-900 border border-slate-700 rounded-lg p-4 flex flex-col items-center">
                ...
                <p className="text-slate-400 text-sm mt-3 text-center px-3">Preview updates live as you customize. Tap hairstyles to see different previews. Fully responsive.</p>
              </div>
            </div>
          </div>
            ...
            <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
              <div className="bg-slate-800 border border-slate-700 rounded-lg max-w-xl w-full p-4 md:p-6">
                ...
                <div className="flex flex-col md:flex-row gap-3">
                  <div className="flex-1">
                    <div className="bg-slate-900 p-3 rounded border border-slate-700 flex items-center justify-center">
...
function HairIcon({ styleId, color = '#333', gender }) {
  // Return small SVG variations per style
  const common = { fill: color }
  switch (styleId) {
    case 'buzz':
      return <svg width="28" height="28" viewBox="0 0 28 28"><rect x="2" y="8" width="24" height="12" rx="6" fill={color} /></svg>
    case 'pompadour':
      return <svg width="28" height="28" viewBox="0 0 28 28"><path d="M2 12c6-8 20-8 24 0v2H2v-2z" fill={color} /></svg>
    case 'slick':
      return <svg width="28" height="28" viewBox="0 0 28 28"><path d="M2 10c8-6 18-6 24 0v4H2v-4z" fill={color} /></svg>
    case 'manbun':
      return <svg width="28" height="28" viewBox="0 0 28 28"><circle cx="20" cy="6" r="4" fill={color} /><path d="M2 12c6-6 14-6 18 0v2H2v-2z" fill={color} /></svg>
    case 'long_male':
      return <svg width="28" height="28" viewBox="0 0 28 28"><path d="M2 10c8-2 18-2 24 0v10H2V10z" fill={color} /></svg>
    case 'curly_male':
      return <svg width="28" height="28" viewBox="0 0 28 28"><path d="M2 10c4-6 20-6 24 0v2H2v-2z" fill={color} /><circle cx="8" cy="8" r="2" fill={color} /></svg>
    case 'bob':
      return <svg width="28" height="28" viewBox="0 0 28 28"><path d="M2 12c6-6 20-6 24 0v8H2v-8z" fill={color} /></svg>
    case 'ponytail':
      return <svg width="28" height="28" viewBox="0 0 28 28"><path d="M2 10c8-4 18-4 24 0v4H2v-4z" fill={color} /><path d="M20 6c4 4 4 8 0 12" stroke={color} strokeWidth="2" fill="none" /></svg>
    case 'braid':
      return <svg width="28" height="28" viewBox="0 0 28 28"><path d="M2 10c8-4 18-4 24 0v6H2v-6z" fill={color} /><path d="M20 14c2 4 2 6 0 8" stroke={color} strokeWidth="2" fill="none" /></svg>
    case 'bun':
      return <svg width="28" height="28" viewBox="0 0 28 28"><circle cx="18" cy="6" r="4" fill={color} /><path d="M2 12c6-6 14-6 18 0v2H2v-2z" fill={color} /></svg>
    case 'long_female':
      return <svg width="28" height="28" viewBox="0 0 28 28"><path d="M2 10c8-2 18-2 24 0v12H2V10z" fill={color} /></svg>
    case 'curly_female':
      return <svg width="28" height="28" viewBox="0 0 28 28"><path d="M2 10c6-6 18-6 24 0v4H2v-4z" fill={color} /><circle cx="8" cy="8" r="2" fill={color} /><circle cx="12" cy="10" r="2" fill={color} /></svg>
    default:
      return <svg width="28" height="28" viewBox="0 0 28 28"><rect x="4" y="8" width="20" height="12" rx="6" fill={color} /></svg>
  }
}
    ...
    <div className="flex flex-col items-center text-center px-2">
      <svg width={avatarStyle.width} height={avatarStyle.height} viewBox={`0 0 ${avatarStyle.width} ${avatarStyle.height}`}>
        ...
        <g transform={`translate(${avatarStyle.width / 2}, ${avatarStyle.height * 0.28})`}>
          ...
          {/* hair - render variant shapes based on hair_style and gender */}
          <g>
...
function renderHairSVG(style, color, headRadius) {
  const h = headRadius
  const common = { fill: color }
  switch (style) {
    case 'buzz':
      return <rect x={-h} y={-h} width={h * 2} height={h * 1.1} rx={h * 0.2} fill={color} />
    case 'pompadour':
      return <path d={`M ${-h} ${-h * 0.4} q ${h * 0.6} ${-h * 1.0} ${h * 1.6} ${-h * 0.2} q ${-h * 0.2} ${h * 0.6} ${-h * 1.6} ${h * 0.4}`} fill={color} />
    case 'slick':
      return <path d={`M ${-h} ${-h * 0.6} q ${h * 0.8} ${-h * 0.6} ${h * 1.6} ${-h * 0.2} v ${h * 0.6} h ${-h * 1.6} z`} fill={color} />
    case 'manbun':
      return <g><circle cx={h * 0.35} cy={-h * 1.05} r={h * 0.35} fill={color} /><path d={`M ${-h} ${-h * 0.1} q ${h * 0.8} ${-h * 0.6} ${h * 1.8} ${-h * 0.1} v ${h * 0.8} h ${-h * 1.8} z`} fill={color} /></g>
    case 'long_male':
      return <path d={`M ${-h * 1.2} ${-h * 0.6} q ${h * 1.6} ${-h * 0.8} ${h * 2.4} ${0} v ${h * 1.6} q ${-h * 0.3} ${h * 0.4} ${-h * 2.4} ${0} z`} fill={color} />
    case 'curly_male':
      return <g>{[ -0.6, -0.2, 0.2, 0.6 ].map((cx, i) => (<circle key={i} cx={h * cx} cy={-h * 0.7 + (i%2)*4} r={h * 0.28} fill={color} />))}</g>
    case 'bob':
      return <path d={`M ${-h * 1.2} ${-h * 0.1} q ${h * 1.2} ${-h * 0.8} ${h * 2.4} ${-h * 0.1} v ${h * 0.9} q ${-h * 0.3} ${h * 0.4} ${-h * 2.4} ${0} z`} fill={color} />
    case 'ponytail':
      return <g><path d={`M ${-h} ${-h * 0.2} q ${h * 0.8} ${-h * 0.6} ${h * 1.6} ${-h * 0.1} v ${h * 0.6} h ${-h * 1.6} z`} fill={color} /><path d={`M ${h * 0.9} ${-h * 0.2} q ${h * 0.8} ${h * 0.6} ${h * 1.0} ${h * 1.6}`} stroke={color} strokeWidth={Math.max(2, h*0.08)} fill="none" strokeLinecap="round" /></g>
    case 'braid':
      return <g>{[0,1,2,3].map(i=> <ellipse key={i} cx={h*0.8 + i* -h*0.18} cy={-h*0.0 + i*8} rx={h*0.22} ry={h*0.12} fill={color} />)}</g>
    case 'bun':
      return <g><circle cx={h*0.4} cy={-h*1.05} r={h*0.36} fill={color} /><path d={`M ${-h} ${-h * 0.1} q ${h * 0.8} ${-h * 0.6} ${h * 1.8} ${-h * 0.1} v ${h * 0.8} h ${-h * 1.8} z`} fill={color} /></g>
    case 'long_female':
      return <path d={`M ${-h * 1.4} ${-h * 0.6} q ${h * 1.8} ${-h * 0.8} ${h * 2.8} ${0} v ${h * 1.8} q ${-h * 0.3} ${h * 0.4} ${-h * 2.8} ${0} z`} fill={color} />
    case 'curly_female':
      return <g>{[-0.7,-0.3,0.1,0.5,0.9].map((x,i)=><circle key={i} cx={h*x} cy={-h*0.7 + (i%2)*6} r={h*0.32} fill={color} />)}</g>
    default:
      return <rect x={-h} y={-h*0.6} width={h*2} height={h*1.1} rx={h*0.3} fill={color} />
  }
}
