...
export default function BalanceDisplay({ userId }) {
  ...
  useEffect(() => {
    ...
    const fetchBalance = async () => {
      try {
        const bal = await dogTokenAPI.getUserBalance(userId)
        setBalance(bal)
        setError(null)
      } catch (err) {
    ...
    const subscription = dogTokenAPI.subscribeToBalance(userId, (newBalance) => {
  ...
  return (
    <div className="bg-white border border-gray-200 p-6 mb-6">
      ...
      <p className="text-3xl font-bold text-black">
        {balance !== null ? balance.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '0'} DOG
      </p>
