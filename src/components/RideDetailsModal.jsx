...
export default function RideDetailsModal({ ride, driver, onClose, onCancelRide, onUpdateStatus, loading }) {
      ...
      {ride.status !== 'completed' && ride.status !== 'cancelled' && (
        <>
          <button
            ...
            disabled={loading}
            className="flex-1 min-w-[100px] px-4 py-2 border border-red-300 text-red-700 rounded-lg hover:bg-red-50 font-medium transition-colors disabled:bg-slate-200 disabled:text-slate-400"
          >
            Cancel
          </button>
          {ride.status === 'in-progress' && (
            <button
              ...
              disabled={loading}
              className="flex-1 min-w-[100px] px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 font-medium transition-colors disabled:bg-slate-400"
            >
              Complete
            </button>
          )}
  ...
  return (
    <ExpandableModal
      ...
      badgeContent={getStatusLabel(ride.status)}
      showBadge={true}
