...
import cryptoBalanceService from '../lib/cryptoBalanceService'
...
export default function HomePage({ userId, userEmail, globalCurrency = 'PHP', setGlobalCurrency, globalCryptocurrency = 'BTC', setGlobalCryptocurrency, onTabChange, onSetTotalCryptoBalance, onShowAuth }) {
  ...
  const [loadingCryptoHoldingsConversion, setLoadingCryptoHoldingsConversion] = useState(false)
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="relative mb-8">
          <button
            ...
            className="absolute -top-8 right-0 flex items-center gap-2 cursor-pointer hover:opacity-100 transition-opacity"
            title="Click to customize quick access cards"
          >
            <span className="text-sm font-medium text-slate-400 opacity-60 hover:opacity-100">Quick Access</span>
            <div className="p-1.5 hover:bg-slate-100 rounded-lg transition-colors text-slate-400 hover:text-slate-600 opacity-60 hover:opacity-100">
      ...
      {showPaymentsModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-2xl max-w-6xl w-full max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b border-slate-200 p-6 flex items-center justify-between">
              <h2 className="text-2xl font-light text-slate-900">Payments</h2>
              <button
            ...
            <div className="p-6">
              <PaymentsHub userId={userId} userEmail={userEmail} globalCurrency={globalCurrency} setGlobalCurrency={setGlobalCurrency} />
            </div>
          </div>
        </div>
      )}
