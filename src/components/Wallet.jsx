            ...
            {!showFiatOnly && (
              <div className="mb-12">
                <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
                  ...
                  <p className="text-sm text-slate-500">{currenciesGrouped.crypto.filter(c => enabledInternal.includes(c.code)).length} available</p>
                </div>

                {currenciesGrouped.crypto.length === 0 ? (
                  <div className="bg-slate-50 border border-slate-200 rounded-lg p-8 text-center">
                    <p className="text-slate-500">No cryptocurrencies available</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    {currenciesGrouped.crypto.filter(c => {
                      // Apply enabled filter
                      if (!enabledInternal.includes(c.code)) return false
                      // Apply selected currency filter
                      if (selectedCurrency && selectedCurrency !== c.code) return false
                        ...
                        <div key={wallet.id} className={`bg-white border border-slate-200 rounded-lg p-5 hover:shadow-lg transition-all hover:border-slate-300 ${isPlaceholder ? 'opacity-75' : ''}`}>
                          ...
                          <div className="flex gap-2">
                            <button
                              ...
                              className="flex-1 py-2 px-3 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition-colors text-xs font-medium"
                            >
                              Add
                            </button>
                            <button
