...
export default function Nearby({ userId, setActiveTab, setCurrentListingSlug }) {
  ...
  useEffect(() => {
    ...
    const handle = setTimeout(async () => {
      ...
      try {
        ...
        console.error('Error searching (realtime):', {
  ...
  async function loadStats() {
    try {
      ...
      // Filter ratings client-side
      const validRatings = (ratingData || []).filter(d => d.rating !== null && d.rating !== undefined)
      const avgRating = validRatings.length > 0
        ? (validRatings.reduce((sum, d) => sum + (d.rating || 0), 0) / validRatings.length).toFixed(1)
        : 0
  ...
  async function loadPendingListings() {
    try {
      ...
      const errorMessage = err?.message || err?.details || String(err) || 'Unknown error'
      console.error('Error loading pending listings:', {
        message: errorMessage,
        status: err?.status,
        code: err?.code,
        details: err?.details
