...
export default function ServicesModal({ 
  ...
  const handleSelect = () => {
    onSelectService(expandedService)
    onClose()
  }
      ...
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col relative z-[10000]">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {/* Service Details */}
            {selectedServiceData && (
              <div className={`rounded-lg p-6 text-white space-y-6 ${getBackgroundColor(expandedService)}`}>
                {/* Service Header */}
                <div>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {selectedServiceData.criteria.map((criterion, idx) => (
                ...
                <div className="border-t border-white border-opacity-20 pt-4">
                  ...
                  <div className="bg-white bg-opacity-10 rounded-lg p-3 space-y-2 text-sm">
                    {selectedServiceData.details.baseFare && (
                      <div className="flex justify-between">
                        <span className="text-opacity-90">Base Fare:</span>
                        <span className="font-semibold">₱{selectedServiceData.details.baseFare}</span>
                      </div>
                    )}
                    {selectedServiceData.details.perKm && (
                      <div className="flex justify-between">
                        <span className="text-opacity-90">Per km:</span>
                        <span className="font-semibold">₱{selectedServiceData.details.perKm}</span>
                      </div>
                    )}
                    {selectedServiceData.details.perKg && (
                      <div className="flex justify-between">
                        <span className="text-opacity-90">Per kg:</span>
                        <span className="font-semibold">₱{selectedServiceData.details.perKg}</span>
                      </div>
                    )}
                    {selectedServiceData.details.maxDocuments && (
                      <div className="flex justify-between text-opacity-90">
                        <span>Max Documents:</span>
                        <span className="font-semibold">{selectedServiceData.details.maxDocuments}</span>
                      </div>
                    )}
                    {selectedServiceData.details.maxDeliveryTime && (
                      <div className="flex justify-between text-opacity-90">
                        <span>Max Delivery Time:</span>
                        <span className="font-semibold">{selectedServiceData.details.maxDeliveryTime} min</span>
                      </div>
                    )}
                    {selectedServiceData.details.requiresCertification && (
                    ...
                    {selectedServiceData.details.insurable && (
