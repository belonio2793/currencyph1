...
export default function Auth({ onAuthSuccess, initialTab = 'login', isModal = false }) {
  ...
  return (
    <div className={`${isModal ? '' : 'min-h-screen bg-gradient-to-br from-slate-50 to-blue-50'} flex ${isModal ? 'items-center' : 'items-start'} justify-center py-2 px-4 sm:px-6 lg:px-8`}>
      <div className={`${isModal ? '' : 'max-w-md'} ${isModal ? '' : 'mt-2 mb-2'}`} style={isModal ? { width: '90%', maxWidth: '1100px' } : { width: '100%' }}>
        {/* Header */}
        {!isModal && (
