...
import { phpToUsd, displayBothCurrencies, formatCurrency, fetchExchangeRate } from '../lib/currencyUtils'
import L from 'leaflet'
import { MapContainer, TileLayer, Marker, Popup, useMapEvents, useMap, Circle, Tooltip } from 'react-leaflet'
import 'leaflet/dist/leaflet.css'
import Auth from './Auth'
import PartnershipNetworkSection from './PartnershipNetworkSection'
  ...
  return (
    <div className={`bg-slate-900 flex flex-col min-h-screen`}>
            ...
            <>
              {locations.length > 0 && (
                <div className={`flex ${isMobile ? 'flex-col gap-1 w-full' : 'items-center gap-2'}`}>
                  <label htmlFor="public-locations-select" className={`text-slate-300 font-medium text-opacity-75 ${isMobile ? 'text-xs' : 'text-sm'}`}>Locations</label>
                  <select
              ...
              {shippingPorts.length > 0 && (
                <div className={`flex ${isMobile ? 'flex-col gap-1 w-full' : 'items-center gap-2'}`}>
                  <label htmlFor="shipping-ports-select" className={`text-slate-300 font-medium text-opacity-75 ${isMobile ? 'text-xs' : 'text-sm'}`}>Ports</label>
                  <select
              ...
              <div className={`flex ${isMobile ? 'flex-col gap-1 w-full' : 'items-center gap-2'}`}>
                <label htmlFor="cities-select" className={`text-slate-300 font-medium text-opacity-75 ${isMobile ? 'text-xs' : 'text-sm'}`}>Cities</label>
                <select
                  id="cities-select"
                  value={selectedCity}
      ...
      <div className={`flex-1 flex ${isMobile || !isAuthenticated ? 'flex-col gap-3' : 'gap-4'} ${isMobile ? 'p-3' : 'p-6'} overflow-hidden`}>
        ...
        {/* Map Section */}
        <div id="planning-map" className={`rounded-lg overflow-hidden border border-slate-700 bg-slate-800 flex flex-col ${isMobile || !isAuthenticated ? 'w-full' : ''} ${isMobile ? 'flex-1 min-h-[calc(100vh-400px)]' : 'flex-1 min-h-[800px]'}`} style={showLocationForm || showAuthModalOnDemand ? { pointerEvents: 'none' } : {}}>
          {/* Map Controls */}
          {isAuthenticated && (
        ...
        {/* Chat Section */}
        <div id="planning-chat" className={`rounded-lg border border-slate-700 bg-slate-800 flex flex-col overflow-hidden ${isMobile || !isAuthenticated ? 'w-full' : 'w-72'} flex-1`}>
      ...
      <div className="mt-24 border-t-8 border-amber-600">
        ...
        <section id="coconuts-ecosystem" className="py-24 px-4 sm:px-6 lg:px-8 bg-white">
          <div className="max-w-7xl mx-auto">
            <div className="mb-16">
              <div className="inline-block mb-4 px-4 py-2 bg-amber-100 text-amber-700 rounded-full text-sm font-semibold">
                PARTNERSHIP NETWORK
        ...
        <section id="coconuts-market" className="py-24 px-4 sm:px-6 lg:px-8 bg-gray-50">
          <div className="max-w-7xl mx-auto">
            <div className="mb-16">
