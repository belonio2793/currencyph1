...
const CARD_CONFIG = {
  ...
  payments: {
    title: 'Payments',
    description: 'Accept and send payments instantly with real-world currencies',
    icon: (
    ...
    colorClasses: 'bg-emerald-100 group-hover:bg-emerald-200 text-emerald-600',
    borderClasses: 'hover:border-emerald-300'
  },
  wallets: {
    title: 'My Wallets',
    ...
    borderClasses: 'hover:border-cyan-300'
  }
}
  ...
  if (isLargeMode) {
    ...
    const getCompactCardColors = (cardKey) => {
      const colorMap = {
        ...
        payments: { bg: 'bg-emerald-50', border: 'border-emerald-200', icon: 'text-emerald-600' }
      }
        ...
        {cardKeys.map((cardKey, index) => {
          ...
          return (
            ...
            >
              <button
                onClick={() => onCardClick && onCardClick(cardKey)}
                className={`cursor-pointer w-full flex items-center gap-3 p-3 rounded-lg border transition-all ${colors.bg} ${colors.border} ${
