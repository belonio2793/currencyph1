...
export default function EquipmentManager({ projectId, onClose, exchangeRate = 0.018 }) {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto bg-white rounded-lg shadow-lg overflow-hidden">
      {/* Header */}
      <div className="sticky top-0 bg-slate-700 text-white p-6 flex items-center justify-between">
        <div>
      ...
      {error && <div className="bg-slate-100 border-b border-slate-200 text-slate-700 p-4 text-sm">{error}</div>}
      {success && <div className="bg-slate-100 border-b border-slate-200 text-slate-700 p-4 text-sm">{success}</div>}
          ...
          <div className="p-8">
            <div className="bg-slate-50 border border-slate-200 rounded-lg p-6">
              ...
              <p className="text-slate-700 mb-4">
                Add equipment items manually to the project.
              </p>
              <button
                onClick={addNewEquipment}
                className="w-full px-6 py-3 bg-slate-700 text-white rounded-lg hover:bg-slate-800 font-medium"
              >
                Add First Equipment
              </button>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="border-b pb-6">
                ...
                {current.id ? (
                  <>
                    <div className="mb-4">
                      <label className="block px-4 py-8 border-2 border-dashed border-slate-300 rounded-lg cursor-pointer hover:border-slate-400 transition-colors text-center">
                        <input
                          type="file"
                          multiple
                          accept="image/*"
                          ...
                          className="hidden"
                        />
                        <div className="text-slate-700 font-medium">Click to upload images</div>
                          ...
                          <div key={img.id} className="relative group">
                            ...
                            <div className="absolute inset-0 bg-black/0 group-hover:bg-black/30 rounded-lg transition-all flex items-center justify-center opacity-0 group-hover:opacity-100">
                              <button
                                ...
                                className="px-2 py-1 bg-slate-700 text-white text-sm rounded"
                              >
                                Delete
                              </button>
                            </div>
        ...
        <div className="border-t bg-slate-50 p-6">
          ...
          <div className="flex gap-3">
            <button
              ...
              disabled={saving}
              className="flex-1 px-6 py-3 bg-slate-700 text-white rounded-lg hover:bg-slate-800 disabled:opacity-50 font-medium"
            >
              {saving ? 'Saving...' : 'Save Equipment'}
            </button>
            <button
              onClick={deleteCurrentEquipment}
              className="px-6 py-3 bg-slate-200 text-slate-700 rounded-lg hover:bg-slate-300 font-medium text-sm"
            >
              Delete
            </button>
            <button
